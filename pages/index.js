...
const IMG = {
  kale_bunch: "/images/kale_bunch.png", kale_leaf: "/images/kale_leaf.png",
  apple_whole: "/images/apple_whole.png", apple_half: "/images/apple_half.png",
  avocado: "/images/avocado.png", pumpkin: "/images/pumpkin.png",
  banana_slice: "/images/banana_slice.png", beet: "/images/beet.png",
};
const Ic = ({ d, sz=16, stroke="currentColor", fill="none", sw=1.6 }) => (
  <svg width={sz} height={sz} viewBox="0 0 24 24" fill={fill} stroke={stroke} strokeWidth={sw} strokeLinecap="round" strokeLinejoin="round"><path d={d}/></svg>
);
const Icons = {
...
function parseReview(raw) {
  ...
  return { intro: raw.substring(0, hits[0].index).trim(), sections: hits.map((h,i) => ({ header: h[1].trim(), content: raw.substring(h.index+h[0].length, i<hits.length-1?hits[i+1].index:raw.length).trim() })) };
}
...
async function extractPDF(file) {
  ...
  let text="";
  for (let i=1;i<=Math.min(pdf.numPages,15);i++) { const c=await(await pdf.getPage(i)).getTextContent(); text+=c.items.map(s=>s.str).join(" ")+"\n"; }
  return text.trim();
}
...
function Dots() {
  return <div style={{display:"flex",gap:5,alignItems:"center",padding:"4px 2px"}}>{[0,1,2].map(i=><div key={i} style={{width:7,height:7,borderRadius:"50%",background:Tk.sprout,opacity:.5,animation:`dot 1.2s infinite ${i*.2}s`}}/>)}</div>;
}
function BotMark() {
  return <div style={{width:28,height:28,borderRadius:"50%",background:Tk.sproutSoft,display:"grid",placeItems:"center",flexShrink:0,overflow:"hidden"}}><Img k="kale_bunch" w={24} h={24}/></div>;
}
  ...
  return (
    <div style={{background:Tk.paper,border:`0.5px solid ${Tk.line}`,borderRadius:14,padding:"14px 16px",marginBottom:10,position:"relative",overflow:"hidden"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8,position:"relative",zIndex:1}}>
        <div style={{width:22,height:22,borderRadius:7,background:bg,color:accent,display:"grid",placeItems:"center"}}>{kind==="strong"?<Icons.Check size={13} sw={2.6}/>:<Icons.Sparkle size={11} sw={2.2}/>}</div>
        <div style={{fontFamily:FONT,fontSize:13,color:Tk.ink,textTransform:"uppercase",letterSpacing:.8}}>{title}</div>
      </div>
...
function CoachMsg({html:htmlContent,raw,isTyping,chips,onChip}) {
  ...
  return (
    <div style={{maxWidth:580}}>
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}><BotMark/><div style={{fontSize:10.5,color:Tk.muted,textTransform:"uppercase",letterSpacing:1.6,fontFamily:FONT}}>{isRev?"Coach · Review":"Coach"}</div></div>
      {isTyping?<Dots/>:isRev?(
        <>
          {(headline||subtitle)&&<div style={{fontFamily:FONT,fontSize:19,lineHeight:1.35,letterSpacing:-.2,color:Tk.ink,marginBottom:14}}>{headline&&<span>{headline}<br/></span>}{subtitle&&<span style={{color:Tk.muted}}>{subtitle}</span>}</div>}
          {review.sections.map(({header,content})=><ReviewBlock key={header} kind={header.toLowerCase().includes("strong")?"strong":"sharpen"} title={header} rawContent={content}/>)}
        </>
      ):<div style={{fontSize:14,lineHeight:1.7,color:Tk.ink2,fontFamily:FONT}} dangerouslySetInnerHTML={{__html:htmlContent}}/>}
      {!isTyping&&chips?.length>0&&<div style={{display:"flex",flexWrap:"wrap",gap:8,marginTop:16}}>{chips.map(({label,display,imgKey,msg})=><button key={label} style={chipBtn()} onClick={()=>onChip(msg||label,display||label)} onMouseEnter={e=>{e.currentTarget.style.background=Tk.sproutSoft;e.currentTarget.style.borderColor=Tk.sprout;}} onMouseLeave={e=>{e.currentTarget.style.background=Tk.paper;e.currentTarget.style.borderColor=Tk.line;}}>{imgKey&&<Img k={imgKey} w={18} h={18}/>}{display||label}</button>)}</div>}
    </div>
  );
}
function Welcome({onAction}) {
  return (
    <div style={{maxWidth:560}}>
      ...
      <div style={{fontFamily:FONT,fontSize:32,lineHeight:1.1,letterSpacing:-.5,color:Tk.ink,marginBottom:14}}>
        Hi there, Farmer.<br/>
        <span style={{color:Tk.sprout,position:"relative",display:"inline-block"}}>
          How can I help you grow
          <svg viewBox="0 0 200 8" preserveAspectRatio="none" style={{position:"absolute",left:0,right:0,bottom:-6,width:"100%",height:8,color:Tk.sprout}}><path d="M2 5 Q 50 1, 100 4 T 198 4" stroke="currentColor" strokeWidth="2.5" fill="none" strokeLinecap="round"/></svg>
        </span>{" "}today?
      </div>
      <div style={{fontSize:14,lineHeight:1.7,color:Tk.ink2,fontFamily:FONT}}>{"I'm your goal-setting coach. I can "}<strong style={{color:Tk.moss}}>draft</strong>{", "}<strong style={{color:Tk.moss}}>refine</strong>{", "}<strong style={{color:Tk.moss}}>review</strong>{", or "}<strong style={{color:Tk.moss}}>check in</strong>{" on your OKRs. Attach a doc and I'll read it."}</div>
      <div style={{display:"flex",flexWrap:"wrap",gap:8,marginTop:18}}>{QUICK_STARTS.map(({label,display,imgKey,msg})=><button key={label} style={chipBtn()} onClick={()=>onAction(msg,display||label)} onMouseEnter={e=>{e.currentTarget.style.background=Tk.sproutSoft;e.currentTarget.style.borderColor=Tk.sprout;}} onMouseLeave={e=>{e.currentTarget.style.background=Tk.paper;e.currentTarget.style.borderColor=Tk.line;}}><Img k={imgKey} w={18} h={18}/>{display||label}</button>)}</div>
    </div>
  );
}
function UserMsg({text,docWords,sheets}) {
  if (text?.startsWith("Attached: ")) {
    const name=text.replace("Attached: ",""), meta=[sheets?`${sheets} sheet${sheets!==1?"s":""}`:null,docWords?`${docWords.toLocaleString()} words`:null].filter(Boolean).join(" · ");
    return <div style={{alignSelf:"flex-end",maxWidth:380}}><div style={{background:Tk.paper,border:`0.5px solid ${Tk.line}`,borderRadius:14,padding:"10px 14px 10px 10px",display:"flex",alignItems:"center",gap:12}}><div style={{width:42,height:42,borderRadius:10,background:Tk.sproutSoft,display:"grid",placeItems:"center",flexShrink:0}}><Img k="apple_half" w={32} h={32}/></div><div style={{flex:1,minWidth:0}}><div style={{fontSize:13,color:Tk.ink,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",fontFamily:FONT}}>{name}</div>{meta&&<div style={{fontSize:11,color:Tk.muted,marginTop:2,fontFamily:FONT}}>{meta}</div>}</div></div></div>;
  }
...
export default function OKRChatbot() {
  ...
  const processFile=useCallback(async(file)=>{
    ...
    try {
      const text=await parseFile(file); if(!text||text.length<20) throw new Error("Document appears empty or image-only.");
      const words=text.split(/\s+/).filter(Boolean).length, trimmed=text.substring(0,6000);
      ...
      callAPI(`ATTACHED DOCUMENT: "${file.name}"\n\nContent:\n${trimmed}\n\n---\n\nI just attached "${file.name}". Please acknowledge what you can see and ask what I would like to do.`,`Attached: ${file.name}`,history,null,null);
    } catch(e) { setDocText(null); setDocName(null); setDocOk(false); alert(e.message); if(fileRef.current)fileRef.current.value=""; }
  },[history,callAPI]);
  ...
  return (
    <>
      <Head>
        ...
        <style>{`
          @font-face { font-family:'NeueKabel'; font-weight:400; font-style:normal; src:url('/fonts/NeueKabel-Regular.otf') format('opentype'); font-display:block; }
          *{box-sizing:border-box;font-family:'NeueKabel',sans-serif;font-weight:400;font-style:normal;}
          ...
          ul{margin:6px 0 4px 18px;padding:0;} li{margin-bottom:3px;}
          ...
          .mi{animation:mi 0.2s ease both} strong{font-weight:400;} button{font-family:'NeueKabel',sans-serif;font-weight:400;}
        `}</style>
      </Head>
      <div style={{minHeight:"100vh",background:Tk.cream,display:"flex",justifyContent:"center",alignItems:"flex-start",padding:"32px 16px"}}>
        <div onDragEnter={onDE} onDragLeave={onDL} onDragOver={onDO} onDrop={onDrop}
          ...
          {drag&&<div style={{position:"absolute",inset:0,zIndex:50,background:`${Tk.sproutSoft}cc`,border:`2px dashed ${Tk.sprout}`,borderRadius:18,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:10,pointerEvents:"none"}}><Img k="apple_half" w={48} h={48}/><div style={{fontFamily:FONT,fontSize:16,color:Tk.moss}}>Drop your document here</div><div style={{fontSize:12,color:Tk.muted}}>PDF, Word, Excel, or plain text</div></div>}
          ...
          <div style={{padding:"14px 22px",borderBottom:`0.5px solid ${Tk.line}`,background:Tk.paper,display:"flex",alignItems:"center",gap:12,flexShrink:0}}>
            <div style={{width:38,height:38,position:"relative",flexShrink:0}}><div style={{position:"absolute",inset:0,borderRadius:"50%",background:Tk.sproutSoft}}/><div style={{position:"absolute",top:3,left:3}}><Img k="kale_bunch" w={32} h={32}/></div></div>
            <div style={{flex:1}}><div style={{fontFamily:FONT,fontSize:19,letterSpacing:-.3,lineHeight:1,color:Tk.ink}}>OKR Coach</div><div style={{fontSize:11,color:Tk.muted,marginTop:4,fontFamily:FONT}}>Goal-setting support for Farmers, people leaders, and teams</div></div>
            <button style={pillBtn()} onClick={restart} onMouseEnter={e=>{e.currentTarget.style.background=Tk.sproutSoft;e.currentTarget.style.borderColor=Tk.sprout;}} onMouseLeave={e=>{e.currentTarget.style.background=Tk.cream;e.currentTarget.style.borderColor=Tk.line;}}><Icons.Restart size={12} sw={2.2}/> New conversation</button>
          </div>
          <div style={{flex:1,overflow:"auto",padding:"30px 28px 16px",display:"flex",flexDirection:"column",gap:24,position:"relative",minHeight:420}}>
            <div style={{position:"absolute",left:-34,bottom:90,width:130,opacity:.10,transform:"rotate(-18deg)",pointerEvents:"none",zIndex:0}}><Img k="kale_leaf" w={130} h={130}/></div>
            <div style={{position:"absolute",right:-28,top:130,width:120,opacity:.10,transform:"rotate(18deg)",pointerEvents:"none",zIndex:0}}><Img k="banana_slice" w={120} h={120}/></div>
            <div style={{position:"absolute",right:-50,bottom:-30,width:180,opacity:.09,transform:"rotate(-8deg)",pointerEvents:"none",zIndex:0}}><Img k="pumpkin" w={180} h={180}/></div>
            <div style={{position:"absolute",left:-22,top:180,width:90,opacity:.10,transform:"rotate(14deg)",pointerEvents:"none",zIndex:0}}><Img k="avocado" w={90} h={90}/></div>
          ...
          <div style={{padding:16,borderTop:`0.5px solid ${Tk.line}`,background:Tk.paper,flexShrink:0}}>
            <div style={{background:Tk.fieldBg,borderRadius:18,padding:14,border:`1px solid ${Tk.line}`}}>
              <textarea ref={inputRef} value={input} disabled={loading} onChange={e=>{setInput(e.target.value);e.target.style.height="auto";e.target.style.height=Math.min(e.target.scrollHeight,120)+"px";}} onKeyDown={onKey} placeholder="Reply to the coach..." rows={1} style={{width:"100%",border:"none",background:"transparent",fontFamily:FONT,fontSize:14,color:input?Tk.ink:Tk.mutedSoft,resize:"none",outline:"none",minHeight:22,maxHeight:120,lineHeight:1.5}}/>
              <div style={{display:"flex",alignItems:"center",gap:8,marginTop:10}}>
                <label style={{...iconBtn(),position:"relative"}}><Icons.Paperclip size={14}/><input ref={fileRef} type="file" accept=".pdf,.docx,.doc,.txt,.xlsx,.xls" onChange={onFile} style={{position:"absolute",inset:0,opacity:0,cursor:"pointer",width:"100%",height:"100%"}}/></label>
                <button style={iconBtn()} onClick={()=>{}}><Icons.Mic size={14}/></button>
                {docOk&&docName&&<div style={{display:"flex",alignItems:"center",gap:5,background:Tk.sproutSoft,borderRadius:8,padding:"3px 8px 3px 6px",fontSize:11,color:Tk.moss,fontFamily:FONT}}><Icons.Sheet size={12} stroke={Tk.moss}/><span style={{maxWidth:120,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{docName}</span><button onClick={clearDoc} style={{background:"none",border:"none",cursor:"pointer",color:Tk.moss,fontSize:12,padding:"0 1px",lineHeight:1,fontFamily:FONT}}>x</button></div>}
                <div style={{flex:1}}/>
                <div style={{fontSize:11,color:Tk.muted,fontFamily:FONT}}>{String.fromCharCode(9166)} to send</div>
                <button onClick={send} disabled={loading||!input.trim()} style={{width:34,height:34,borderRadius:11,background:loading||!input.trim()?Tk.lineSoft:Tk.moss,color:"#fff",border:"none",display:"grid",placeItems:"center",cursor:loading||!input.trim()?"default":"pointer",transition:"background 0.13s"}}><Icons.Send size={14} stroke="#fff"/></button>
              </div>
            </div>
          </div>
        </div>
